const images = [            // 이미지 배열
            ...
            const imagesPerpage = 2;    // 한 페이지의 표시될 이미지 수
            let currentPage = 0;        // 현재 페이지를 나타내는 변수 // 초기값은 0으로 설정하여 첫 번째 페이지를 표시함

            const imageContainer = document.querySelector('.img-container'); //이미지가 표시될 div공간 // 클래스라 .이붙음 id는 # 
            ...
            displayImg(page) : page는 현재 페이지 번호를 나타냄
                호출될 때 마다 페이지를 전환하고 해당 페이지의 이미지를 표시하는 데 사용
            ...
            */
            function displayImg(page){
                ...
                **핵심** 
                ...
                // 이미지 배열('images')에 시작 인덱스와 종료 인덱스 사이에 이미지를 추출해서
            ...
            function updateButtons(){
                ...
                */
                ...
                (currentPage + 1) * imagesPerpage = 다음 페이지에 해당하는 이미지의 첫번째 인덱스
                        ...
                        다음페이지에 이미지가 더이상 없을 떄 다음 버튼을 비활성화 하겠음
                */
            ...
            prevButton.addEventListener('click',()=>{
                ...
                updateButtons();            
                // 이전 버튼이 첫 번째 페이지에서 클릭하게 되면 더이상 이전 버튼으로 가지 못하도록 비활성화
                }
